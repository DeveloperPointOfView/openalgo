...
def invalidate_session_if_invalid(f):
    ...
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Allow transitional state after username/password login where
        # session['user'] is set but broker OAuth isn't completed yet.
        # In this state we must not clear the session, otherwise the UI
        # is stuck bouncing between pages during the login flow.
        try:
        ...
        transitional_login = ('user' in session and not session.get('logged_in'))
        broker_flow = (path == '/auth/broker' or path.startswith('/auth/broker/'))
        public_flow = path in ['/', '/download', '/faq']

        if transitional_login and (broker_flow or public_flow):
            # Skip invalidation during transitional login on public pages and broker page
            return f(*args, **kwargs)
