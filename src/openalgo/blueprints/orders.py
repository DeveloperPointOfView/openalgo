...
from openalgo.database.auth_db import get_auth_token, get_api_key_for_tradingview
from openalgo.database.settings_db import get_analyze_mode
from openalgo.utils.session import check_session_validity
from openalgo.services.place_smart_order_service import place_smart_order
from openalgo.services.close_position_service import close_position
from openalgo.services.orderbook_service import get_orderbook
from openalgo.services.tradebook_service import get_tradebook
from openalgo.services.positionbook_service import get_positionbook
from openalgo.services.holdings_service import get_holdings
from openalgo.utils.logging import get_logger
from openalgo.limiter import limiter
import csv
import io
import os
...
def orderbook():
    ...
    # Allow password-only login for dev/testing (no broker OAuth required)
    password_only_login = os.getenv('ALLOW_PASSWORD_ONLY_LOGIN', 'FALSE').upper() == 'TRUE'

    if auth_token is None and not password_only_login:
        logger.warning(f"No auth token found for user {login_username}")
        return redirect(url_for('auth.logout'))
    
    # For password-only mode, show empty orderbook
    if password_only_login and auth_token is None:
        logger.info(f"Password-only login mode active for user {login_username} - showing empty orderbook")
        order_data = []
        order_stats = {}
    ...
    if get_analyze_mode():
        ...
        if api_key:
            ...
            order_data = []
            order_stats = {}
            flash("Analyzer mode is ON. Add an API key (Settings → API Keys) to use sandbox orderbook.", "info")
            return render_template('orderbook.html', order_data=order_data, order_stats=order_stats)
    else:
        # Use live broker
...
def tradebook():
    ...
    # For password-only mode, show empty tradebook
    if password_only_login and auth_token is None:
        logger.info(f"Password-only login mode active for user {login_username} - showing empty tradebook")
        trade_data = []
    ...
    if get_analyze_mode():
        ...
        if api_key:
            ...
            trade_data = []
            flash("Analyzer mode is ON. Add an API key (Settings → API Keys) to use sandbox tradebook.", "info")
            return render_template('tradebook.html', trade_data=trade_data)
    else:
        # Use live broker
...
def positions():
    ...
    # For password-only mode, show empty positions
    if password_only_login and auth_token is None:
        logger.info(f"Password-only login mode active for user {login_username} - showing empty positions")
        position_data = []
    ...
    if get_analyze_mode():
        ...
        if api_key:
            ...
            position_data = []
            flash("Analyzer mode is ON. Add an API key (Settings → API Keys) to use sandbox positions.", "info")
            return render_template('positions.html', position_data=position_data)
    else:
        # Use live broker
...
def holdings():
    ...
    if get_analyze_mode():
        ...
        if api_key:
            ...
            holdings_data = []
            portfolio_stats = {}
            flash("Analyzer mode is ON. Add an API key (Settings → API Keys) to use sandbox holdings.", "info")
...
def close_all_positions():
    ...
    try:
        ...
        from openalgo.database.auth_db import get_api_key_for_tradingview
        from openalgo.database.settings_db import get_analyze_mode
