...
from openalgo.services.funds_service import get_funds
...
def dashboard():
    ...
    # For password-only mode, show a placeholder dashboard
    if password_only_login and AUTH_TOKEN is None:
        logger.info(f"Password-only login mode active for user {login_username} - showing placeholder dashboard")
        margin_data = {
            'availablecash': '0.00',
            'collateral': '0.00',
            'utiliseddebits': '0.00'
    ...
    if get_analyze_mode():
        ...
        if api_key:
            ...
            flash("Analyzer mode is ON. Add an API key (Settings → API Keys) to use sandbox balances.", "info")
            return render_template('dashboard.html', margin_data=margin_data)
    else:
        # Use live broker
