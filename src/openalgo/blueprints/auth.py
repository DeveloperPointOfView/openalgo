...
from openalgo.extensions import socketio
import os
from openalgo.database.auth_db import upsert_auth, auth_cache, feed_token_cache
from openalgo.database.user_db import authenticate_user, User, db_session, find_user_by_username, find_user_by_email  # Import the function
from openalgo.database.settings_db import get_smtp_settings, set_smtp_settings
from openalgo.utils.email_utils import send_test_email, send_password_reset_email
from openalgo.utils.email_debug import debug_smtp_connection
import re
from openalgo.utils.session import check_session_validity, get_session_expiry_time, set_session_login_time
import secrets
...
def login():
    ...
    if request.method == 'GET':
        ...
        if authenticate_user(username, password):
            ...
            # If allowed via env, mark fully logged in (dev/local use only)
            if os.getenv('ALLOW_PASSWORD_ONLY_LOGIN', 'FALSE').upper() == 'TRUE':
                ...
                logger.info("Password-only login enabled by env; session marked logged_in=True")
                return jsonify({'status': 'success', 'next': '/dashboard'}), 200
            # Redirect target is decided on client; broker page will forward to dashboard if logged_in
...
def broker_login():
    ...
    if request.method == 'GET':
        ...
        from openalgo.utils.auth_utils import mask_api_credential
...
def logout():
    if session.get('logged_in'):
        ...
        try:
            from openalgo.database.master_contract_cache_hook import clear_cache_on_logout
