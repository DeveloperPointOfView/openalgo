...
from openalgo.database.master_contract_status_db import get_status, check_if_ready
...
def get_master_contract_status():
    ...
    try:
        ...
        if not broker:
            # In password-only login mode, return a placeholder status
            if password_only_login:
                return jsonify({
                    'status': 'info',
                    'message': 'Dev mode - no broker connected',
                    'total_symbols': 0
                }), 200
            else:
                return jsonify({
...
def get_cache_status():
    ...
    try:
        from openalgo.database.token_db_enhanced import get_cache_stats
...
def get_cache_health():
    ...
    try:
        from openalgo.database.master_contract_cache_hook import get_cache_health
...
def reload_cache():
    ...
    try:
        ...
        from openalgo.database.master_contract_cache_hook import load_symbols_to_cache
...
def clear_cache():
    ...
    try:
        from openalgo.database.token_db_enhanced import clear_cache as clear_symbol_cache
