...
from openalgo.utils.config import get_broker_api_key, get_broker_api_secret, get_login_rate_limit_min, get_login_rate_limit_hour
from openalgo.utils.auth_utils import handle_auth_success, handle_auth_failure
...
def broker_callback(broker,para=None):
    ...
    if broker == 'fivepaisa':
        ...
        if request.method == 'GET':
            ...
            from openalgo.utils.httpx_client import get_httpx_client
        ...
        if request.method == 'GET':
            ...
            if token_id:
                ...
                if auth_token:
                    ...
                    from openalgo.broker.dhan.api.funds import test_auth_token
                ...
                # First time coming from openalgo.broker.html - redirect to initiate OAuth
        ...
        if request.method == 'GET':
            ...
            from openalgo.broker.definedge.api.auth_api import login_step1
            ...
            # Handle OTP verification
                ...
                from openalgo.broker.definedge.api.auth_api import authenticate_broker
    ...
    if auth_token:
        ...
        if broker in ['angel', 'compositedge', 'pocketful', 'definedge', 'dhan']:
            ...
            if broker == 'compositedge' and 'user' not in session:
                ...
                from openalgo.database.user_db import find_user_by_username
...
def dhan_initiate_oauth():
    ...
    from openalgo.broker.dhan.api.auth_api import generate_consent, get_login_url
